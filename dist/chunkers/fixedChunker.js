...
export class FixedChunker {
    ...
    constructor(options) {
        ...
        if (this.chunkOverlap >= this.chunkSize) {
            throw new Error("Chunk overlap must be less than chunk size to ensure progression.");
        }
        if (this.chunkSize <= 0) {
    ...
    chunk(text, metadata = {}) {
        ...
        while (currentTokenStartIndex < tokens.length) {
            ...
            const currentTokenEndIndex = Math.min(currentTokenStartIndex + this.chunkSize, tokens.length);
            // Slice the tokens array to get the tokens for the current chunk.
            const chunkTokens = tokens.slice(currentTokenStartIndex, currentTokenEndIndex);
...
//# sourceMappingURL=fixedChunker.js.map